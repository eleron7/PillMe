...
class PasswordManager {
    static let pwManager = PasswordManager()
    ...
    //login 함수
    func findPW(userID: String, userTel: String) -> String
    {
        var query = "SELECT userPW FROM member WHERE userid = '"+userID+"' AND userTEL = '"+userTel+"'"
        ...
        URLSession.shared.dataTask(with: urlRequst, completionHandler: { (data, response, error) in
            ...
            do{
                ...
                {
                    self.findPW = String(self.userModels.first?.userPW ?? "")
                }
            } catch {
