...
class PillDelteManager {
    static let pillDeleteManager = PillDelteManager()
    var run = true
    //생성자
    init() {
    }
    ...
    //약 정보를 삭제하는 함수
    func deletePillInfo(modulenum: String){
        var query = "DELETE FROM t, p USING PillTime AS t LEFT JOIN PillList AS p ON t.ModuleNum = p.ModuleNum WHERE p.ModuleNum ='"+modulenum+"'"
