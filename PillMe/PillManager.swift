...
class PillManager {
    static let pillManager = PillManager()
    var pillModels : [pillModel] = []
    var run = true
    //생성자
    init() {
    }
    ...
    //약 정보를 가져오는 함수
    func selectPillInfo(userID: String) -> [pillModel]{
        var query = "SELECT * FROM PillList WHERE PillMaster = '"+userID+"'"
        ...
        URLSession.shared.dataTask(with: urlRequst, completionHandler: { (data, response, error) in
            ...
            print("약 데이터를 성공적으로 다운로드 했습니다!")
            print(data)
            let jsonString = String(data: data, encoding: .utf8)
            print(jsonString)
                    
            do{
                self.pillModels = try JSONDecoder().decode( [pillModel].self, from: data )
            } catch {
