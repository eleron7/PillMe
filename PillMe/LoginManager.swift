...
class LoginManager {
    static let loginManaer = LoginManager()
    ...
    {
        var query = "SELECT * FROM member WHERE userid = '"+userID+"' AND userPW = '"+userPW+"'"
        ...
        URLSession.shared.dataTask(with: urlRequst, completionHandler: { (data, response, error) in
            ...
            do{
                ...
                if self.userModels.first?.userID == userID && self.userModels.first?.userPW == userPW
                {
                    self.loginOK = true
                }
                else
            ...
            {
                self.loginName = self.userModels[0].userNAME ?? ""
            }
