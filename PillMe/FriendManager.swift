...
class FriendManager {
    ...
    var FriendListResult : [FriendListModel] = []
    ...
    //나와 친구인 계정을 출력하는 함수
    func getFriendList(userID: String) -> [FriendListModel]{
        var query = "SELECT * FROM FriendList WHERE FriendMaster = '"+userID+"'"
        ...
        URLSession.shared.dataTask(with: urlRequst, completionHandler: { (data, response, error) in
            ...
            do{
                self.FriendListResult = try JSONDecoder().decode( [FriendListModel].self, from: data )
            } catch {
